...
- `DONE(host/session/input/policy)`: headless OSC 52 read execution supports a suspending session-bound provider, one active request, strict bounded replies, current-policy checks, deadline/cancellation with bounded timeout-reply commitment, busy and execution audits, and owned output reservation. PTY/workspace listeners route reads to the owning session/tab and await workspace publication within the original deadline; failed publication or cancellation cannot revive a read. Negative replies work without a provider; response denial suppresses them. The [feature map](terminal-feature-map.md#4-query-response-channels) defines the supported contract.
