...
- `TODO(host/ui)`: IntelliJ completion-provider wiring, source ranking, Fig-style spec corpus import, session MRU, profile/directory history indexing, live triggers, full replacement-range command-line semantics, path/current-directory providers, and IntelliJ-native popup styling remain host-owned follow-up work. The reusable Swing terminal popup surface, keyboard/mouse selection, acceptance callback, dependency-free spec-backed completion foundation, standalone spec-backed provider wiring, and standalone/IntelliJ enablement settings exist. Shell-returned completions are intentionally deferred.
