...
- `TODO(host/ui/policy)`: OSC 52 selection targets other than the host clipboard remain unsupported. Product writes act only when the selection is empty or includes `c`; primary/secondary selections and cut buffers need explicit platform mappings. Clipboard reads are tracked in the [read/query gap](#session-transport-rendering-and-host-integration-gaps).
...
- `TODO(host/ui/policy)`: complete IntelliJ OSC 52 read integration and product default migration. Standalone now provides ordered clipboard/primary-selection fallback, native capability detection, one native read across windows retained through actual completion, startup write/read ordering, and cancellable per-pane consent with a persistent terminal block. Shared Swing mechanics have deterministic selection, native-blocking, consent, and byte-stream lifecycle coverage. IntelliJ still needs its client-bound provider, asynchronous pane readiness, posted-write ordering, and consent wiring. Product defaults still allow writes and deny reads; Ask becomes the new-install read default only with working providers and consent, preserving existing Deny choices. Native interoperability validation remains required before closing the product read gap.
