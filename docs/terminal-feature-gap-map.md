...
- `TODO(host/ui/policy)`: supply standalone and IntelliJ OSC 52 read providers through the completed PTY/workspace bridge. Both settings UIs expose Allow/Ask, but neither product supplies native reads or read-consent UI yet. Implement ordered selection fallback, native primary-selection capability detection, bounded native workers that remain occupied after cancellation, startup pane readiness, write/read causality, lifecycle-bound consent, and settings migration. Product defaults still allow writes and deny reads. Ask becomes the new-install read default only with working providers and consent; preserve existing Deny choices. Finish native interoperability validation before closing the product read gap.
