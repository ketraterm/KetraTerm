...
### OSC Protocols
- `DONE(parser/host/session/pty/workspace/ui/policy)`: OSC 52 clipboard write requests are bounded, parsed, denied by default, size-checked, governed by session-wide permissions, surfaced as content-free host audit events, forwarded as decoded text to product-host clipboard callbacks only when policy allows the write, and forwarded to compact standalone/IntelliJ confirmation dialogs when policy requires a prompt. Product prompts identify an application in the named terminal without claiming its origin, using character counts for text writes, and clear-clipboard wording for empty writes instead of exposing raw OSC 52 selection tokens.
- `TODO(host/ui/policy)`: OSC 52 writes to primary/secondary selections and cut buffers, and reads from secondary selections/cut buffers, remain unsupported. Product writes act only when the selection is empty or includes `c`. Reads support `c`/`s` and native `p` when available; remaining read rollout work is tracked in the [read/query gap](#session-transport-rendering-and-host-integration-gaps).
...
- `DONE(policy/host/ui)`: OSC 52 clipboard policy covers session-wide deny/prompt/allow write decisions, including nested SSH output, deny-by-default read/query decisions, decoded payload limits, malformed payload rejection, content-free audit events, allowed writes through product-host clipboard callbacks, and prompt-mode write confirmation in standalone and IntelliJ hosts. Product writes act only when the selection is empty or includes host clipboard `c`; an audit decision of `ALLOWED_BY_POLICY` for another selection or for a read request does not imply execution.
- `DONE(host/session/input/policy)`: headless OSC 52 read execution supports a suspending session-bound provider, one active request, strict bounded replies, current-policy checks, deadline/cancellation with bounded timeout-reply commitment, busy and execution audits, and owned output reservation. PTY/workspace listeners route reads to the owning session/tab. Workspace publication precedes output delivery, preserving startup write/read ordering; failed publication or startup closes the prepared session. Negative replies work without a provider; response denial suppresses them. The [feature map](terminal-feature-map.md#4-query-response-channels) defines the supported contract.
- `TODO(host/ui/policy)`: complete OSC 52 native interoperability validation. Standalone and IntelliJ providers now implement ordered clipboard/primary-selection fallback, bounded native access through actual completion, startup write/read ordering, and cancellable per-pane consent through the same standard dialogs as writes. The shared standard-dialog contract also serves terminal-close and ordinary message dialogs, with common validation and escaped text. Clipboard wording and consent lifecycle replace the custom read overlay; product dialog tests cover decisions, disposal, initial focus requests with Deny selected, and standalone arrow/Tab traversal without accidental approval. IntelliJ uses public APIs with plugin-owned per-client service lifetimes, validates application/project identity across reads, rejects reconnect replacements, waits for asynchronous pane attachment, and retires reads on client/project disposal. Shared and IDE-fixture tests cover selection, native blocking, real product dialogs under the clipboard dispatcher (including cancellation and write consent), client identity and replacement, attachment, cancellation, and byte-stream ordering. Fresh product settings now use Ask while saved choices and legacy read omissions retain their permissions. TOML/XML round-trip tests cover defaults, omissions, invalid values, unrelated edits, and resets; IntelliJ always persists the read permission. Installations without any stored IDE component are indistinguishable from fresh settings and use Ask. An exact-byte terminal harness and Windows ConPTY Allow/Deny tests with a known provider cover the native PTY return path; session regressions also verify UTF-8 boundaries and recovery after failed reads. Actual native clipboard/platform/client validation remains required before closing this gap; follow the [acceptance guide](osc52-testing.md).
...
- `DONE(parser/host/session/policy)`: [bounded OSC 52 writes](terminal-feature-map.md#bounded-osc-52-writes) derive a temporary encoded budget from the active host decoded-byte policy, enabling eligible writes up to the default 1 MiB decoded ceiling. Tests cover buffer release, chunking, precise bounds, malformed data, all supported permissions, policy changes during transfer, and shutdown; ordinary metadata and clipboard reads retain their existing behavior.
