...
- `TODO(completion/ranking)`: introduce an explicit source-aware ranking context for command position, subcommand position, option name/value position, expected argument kind, command family, and source kind so future standalone and IntelliJ dynamic providers can share ranking rules without embedding host-specific logic in the shared engine.
