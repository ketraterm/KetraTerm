...
- `TODO(host/ui)`: IntelliJ completion-provider wiring, production ranking signals, Fig-style spec corpus import, session MRU, profile/directory history indexing, live triggers, path/current-directory providers, and IntelliJ-native popup styling remain host-owned follow-up work. The reusable Swing terminal popup surface, keyboard/mouse selection, acceptance callback, range-aware Swing acceptance, dependency-free layered completion foundation, deterministic source merge/ranking/deduplication, standalone spec-backed provider wiring, and standalone/IntelliJ enablement settings exist. Shell-returned completions are intentionally deferred.
