...
- `DONE(host/session/input/policy)`: headless OSC 52 read execution supports a suspending session-bound provider, one active request, strict bounded replies, current-policy checks, deadline/cancellation with bounded timeout-reply commitment, busy and execution audits, and owned output reservation. Negative replies work without a provider; response denial suppresses them. The [feature map](terminal-feature-map.md#4-query-response-channels) defines the supported contract.
- `TODO(host/ui/policy)`: wire OSC 52 read providers through PTY/workspace into standalone and IntelliJ. Both settings UIs expose Allow/Ask, but neither product supplies native reads or read-consent UI yet. Implement ordered selection fallback, native primary-selection capability detection, bounded native workers that remain occupied after cancellation, startup host readiness, write/read causality, lifecycle-bound consent, and settings migration. Product defaults still allow writes and deny reads. Ask becomes the new-install read default only with working providers and consent; preserve existing Deny choices. Finish native interoperability validation before closing the product read gap.
