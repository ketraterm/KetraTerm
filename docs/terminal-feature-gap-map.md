...
- `TODO(host/ui)`: IntelliJ completion-provider wiring through a plugin-owned adapter, Fig-style spec corpus import, richer command-family ranking once imported specs are available, live triggers, path/current-directory providers, and IntelliJ-native popup styling remain host-owned follow-up work. The reusable Swing terminal popup surface, keyboard/mouse selection, acceptance callback, range-aware Swing acceptance, dependency-free layered completion foundation, deterministic source merge/ranking/deduplication, bounded in-memory session MRU source, spec-declared path argument metadata, command-aware standalone path completion with trailing-space argument semantics, standalone spec/MRU provider wiring with request context, compact standalone exact command/shape/source-feedback stats, source-specific feedback-aware ranking, and standalone/IntelliJ enablement settings exist. Shell-returned completions are intentionally deferred.
