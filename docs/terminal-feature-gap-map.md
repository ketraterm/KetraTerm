...
- `DONE(host/session/ui)`: DECCOLM switches logical columns independently of window-resize permission, with ordered connector synchronization and optional host window resizing. Fixed-pane behavior is described under [Column Toggles](terminal-feature-map.md#1-terminal-protocols--control-sequences).
