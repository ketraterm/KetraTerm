...
- `TODO(host/ui/policy)`: complete OSC 52 native interoperability validation. Standalone and IntelliJ providers now implement ordered clipboard/primary-selection fallback, bounded native access through actual completion, startup write/read ordering, and cancellable per-pane consent. IntelliJ uses public APIs with plugin-owned per-client service lifetimes, validates application/project identity across reads, rejects reconnect replacements, waits for asynchronous pane attachment, and retires reads on client/project disposal. Shared and IDE-fixture tests cover selection, native blocking, consent, client identity and replacement, attachment, cancellation, and byte-stream ordering. Fresh product settings now use Ask while saved choices and legacy read omissions retain their permissions. TOML/XML round-trip tests cover defaults, omissions, invalid values, unrelated edits, and resets; IntelliJ always persists the read permission. Installations without any stored IDE component are indistinguishable from fresh settings and use Ask. Real platform/client validation remains required before closing this gap.
