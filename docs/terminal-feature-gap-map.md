...
- `DONE(parser/policy)`: current OSC/DCS families have explicit collection ceilings and overflow/recovery semantics in the [payload resource contract](terminal-feature-map.md#oscdcs-payload-resource-contract), including bounded unknown-family discard, hyperlink context clearing on completed overflow, and parser-to-host boundary tests. The 4 KiB absolute capacity is unchanged.
- `TODO(parser/policy)`: larger clipboard transfers and graphics require separate bounded storage/transfer designs. Graphics acceptance additionally needs APC where applicable, decoded/decompressed image bounds, and retained-image budgets; current family ceilings alone do not enable graphics.
