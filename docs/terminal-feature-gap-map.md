...
- `TODO(host/ui/policy)`: complete OSC 52 product default migration and native interoperability validation. Standalone and IntelliJ providers now implement ordered clipboard/primary-selection fallback, bounded native access through actual completion, startup write/read ordering, and cancellable per-pane consent. IntelliJ uses public APIs with plugin-owned per-client service lifetimes, validates application/project identity across reads, rejects reconnect replacements, waits for asynchronous pane attachment, and retires reads on client/project disposal. Shared and IDE-fixture tests cover selection, native blocking, consent, client identity and replacement, attachment, cancellation, and byte-stream ordering. Product defaults still allow writes and deny reads; migrate new-install reads to Ask while preserving existing Deny choices. Real platform/client validation remains required before closing this gap.
