...
- `TODO(host/ui)`: shell suggestion providers, ranking, command-line replacement semantics, and IntelliJ-native popup styling remain host-owned follow-up work. The reusable Swing terminal popup surface, keyboard/mouse selection, acceptance callback, and standalone/IntelliJ enablement settings exist.
