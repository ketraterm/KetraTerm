...
- **xterm Key Enhancements**: modifyOtherKeys modes 1 and 2, compact CSI-u layout (`formatOtherKeys=1`), and policy-gated query/explicit-disable controls for the implemented modifyOtherKeys resource (`CSI ? 4 m`, `CSI > 4 n`).
- **Kitty Keyboard Protocol**: High-fidelity key encoding supporting progressive flags:
  ...
  - The input vocabulary and encoder implement event types (`2`), alternate keys (`4`), and associated text (`16`), including text-only key code `0`; these flags remain unadvertised until a rich host can truthfully provide complete lifecycle, layout, and IME metadata.
  - The Swing adapter reports press/repeat/release for AWT-visible non-text physical keys using preallocated key-state storage. Its `KEY_TYPED` printable path intentionally does not invent physical identity or release metadata.
...
KetraTerm includes a pure Kotlin command-line completion pipeline with zero-allocation matching, structured concurrency, and privacy-preserving learning.
...
- **Bounded Local Learning**: Exact-command executions and suggestion feedback adjust matching outcomes with bounded context and recency boosts. The engine derives history and safe unknown-command tokens from the same product-lifetime store and captures one immutable learned view per request. Persistence is opt-in and uses the fixed product-owned `command-completion-stats-v2.tsv` path. Learning updates bounded memory synchronously; one conflated worker hydrates once, checkpoints dirty state every 30 seconds, and forces the final dirty write at shutdown. Persisted sanitized aggregates include exact command text and optional profile/working-directory context. Leading-whitespace commands and common secret patterns are filtered from disk, but still participate in the bounded in-memory learning used by the current product lifetime. This cannot guarantee that every argument, path, URL, or secret is excluded. Base64URL fields are encoded, not encrypted.
