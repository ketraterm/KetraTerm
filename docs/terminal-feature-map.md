...
- **DEC Rectangular Areas**: VT400/VT420 rectangular erase (`DECERA` / `CSI Pt;Pl;Pb;Pr $ z`), selective erase (`DECSERA` / `CSI Pt;Pl;Pb;Pr $ {`), fill (`DECFRA` / `CSI Pch;Pt;Pl;Pb;Pr $ x`), overlap-safe copy (`DECCRA` / `CSI Pts;Pls;Pbs;Prs;Pps;Ptd;Pld;Ppd $ v`), visual-attribute change (`DECCARA` / `CSI Pt;Pl;Pb;Pr;Ps... $ r`), and visual-attribute reversal (`DECRARA` / `CSI Pt;Pl;Pb;Pr;Ps... $ t`). `DECSACE` (`CSI Ps * x`) selects wrapped stream extent (`0`/`1`) or exact rectangle extent (`2`) for attribute operations. Rectangles respect origin and active margins; copy accepts only the active single page (`0` omitted or `1`).
