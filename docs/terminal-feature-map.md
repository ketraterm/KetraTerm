...
- **Completion Engine Foundation**: Dependency-free pure Kotlin completion APIs define command-line requests, explicit replacement-range candidates, candidate kinds, command/option specs, shell-word tokenization, deterministic spec-backed evaluation, and a small curated bootstrap spec set for common developer CLIs. The standalone host wires that engine into the reusable shell-suggestion provider contract for host-triggered popup requests, filtering candidates that require unsupported after-cursor replacement. The engine does not spawn shells, parse terminal output, perform I/O, or depend on Swing/IntelliJ/session/runtime modules.
