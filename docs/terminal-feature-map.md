...
- **Bounded Local Learning**: Exact-command executions and suggestion feedback adjust matching outcomes with bounded context and recency boosts. Persistence is opt-in and uses the fixed product-owned `command-completion-stats-v1.tsv` path. Learning updates bounded memory synchronously and feeds a debounced, conflated latest-state writer; an internal passive repository owns strict bounded file I/O. Persisted sanitized aggregates include exact command text and optional profile/working-directory context. Leading-whitespace commands and common secret patterns are filtered, but this cannot guarantee that every argument, path, URL, or secret is excluded. Base64URL fields are encoded, not encrypted. Session MRU and unknown executable token learning remain in memory per tab.
