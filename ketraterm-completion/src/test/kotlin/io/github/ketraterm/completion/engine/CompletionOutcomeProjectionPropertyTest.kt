...
class CompletionOutcomeProjectionPropertyTest {
    ...
    @Test
    fun `hostile replacement ranges are rejected or use deterministic fallback grouping`() {
        ...
        for ((caseIndex, range) in invalidRanges.withIndex()) {
            ...
            assertNull(request.commandLineAfterCandidate(first), "Invalid projection accepted for case=$caseIndex range=$range")
            ...
            assertEquals(firstResult, completionEngine.complete(request), "Fallback ordering is not deterministic for case=$caseIndex")
        }
        ...
        for (shellSyntax in listOf(TerminalShellSyntax.POSIX, TerminalShellSyntax.POWERSHELL)) {
            repeat(FUSION_CASE_COUNT) { caseIndex ->
                ...
                val completionEngine =
                    engine(
                        listOf(
                            candidate("git switch $wholeQuote", 0, commandLine.length, "history", TerminalCompletionCandidateKind.HISTORY),
                        ),
                        listOf(candidate("switch $tokenQuote", 4, commandLine.length, "spec", TerminalCompletionCandidateKind.SUBCOMMAND)),
                    )

                val candidates = completionEngine.complete(request)

                assertEquals(1, candidates.size, "Equivalent outcome did not fuse for syntax=$shellSyntax case=$caseIndex")
                assertEquals(
                    candidates,
                ...
                val tokens = TerminalCommandLineTokenizer.parse(projected, projected.length, shellSyntax).tokens.map { it.text }
                assertEquals(
    ...
    fun `generated distinct arguments case variants and option order never fuse`() {
        repeat(DISTINCT_CASE_COUNT) { caseIndex ->
            ...
            assertEquals(2, branchEngine.complete(branchRequest).size, "Case-sensitive arguments fused for case=$caseIndex")
            ...
            sources =
                candidateGroups.mapIndexed { index, candidates ->
                    TerminalCompletionSourceEntry(
                        source = TerminalCompletionSource { candidates },
                        priority = index,
                    )
                },
        )
