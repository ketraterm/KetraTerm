...
- Established OSC 52 read-query admission rules: validate the xterm selection alphabet before policy evaluation, require terminal-response permission independently of read permission, and use one policy snapshot for clipboard decisions and audits. Added byte-stream regressions for permission combinations, selector preservation/rejection, chunked policy changes, and independent write permissions. Clipboard read execution and replies remain pending.
...
- Replaced clipboard/title local-versus-remote permissions with session-wide `TerminalClipboardPolicy.writePermission` and `TerminalTitlePolicy.permission`. Removed origin enums, audit origin, executable-name classification, and the unused clipboard `ALLOWLIST` mode/marker. Clipboard choices are Deny / Ask / Allow; persisted `allowlist` values fall back to allow-for-write/deny-for-read defaults. Both products now default to allowed clipboard writes, denied reads, and bounded title updates for every session, including nested SSH. Simplified settings and prompts, renamed persisted keys (old split keys fall back to the new defaults), and added host-metadata isolation and settings regressions.
