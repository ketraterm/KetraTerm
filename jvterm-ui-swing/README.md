...
1. `SwingTerminal` uses an `AtomicBoolean` flag (`renderPending`) to coalesce incoming frame signals.
2. Only one event pass is scheduled on the Event Queue at a time. The flag is atomically cleared just as execution begins.

### `SwingScrollModel` (Smooth Scrollback)
...
* **Host Scrollbar Bridge:** `SwingTerminal.viewportState()` exposes terminal-native scrollback coordinates for external scrollbar adapters. `scrollToScrollbackOffset`, `scrollToLiveViewport`, and `scrollViewportBy` let standalone or IDE hosts drive the same internal scroll model without owning render-cache policy.

### `SwingRepaintPlanner` (Selective Redraws)
Rather than repainting the entire terminal component, `SwingRepaintPlanner` compares the active frame with the last-known painted state to identify the minimal bounding boxes that require updates:
...
* **`SwingHostServices`:** Supplies non-render host services such as UI dispatch, clipboard access, explicit hyperlink activation, and viewport-change notifications. Rendering still consumes immutable settings and packed ARGB palettes, while IntelliJ or standalone hosts provide their own platform adapters outside the paint loop.
...
2. **Selective Redraws:** Validates that `SwingRepaintPlanner` isolates dirty rows and avoids full component repaints.
