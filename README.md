...
* **Advanced Keyboard Shortcuts**: Supports xterm `modifyOtherKeys`, compact CSI-u, and Kitty keyboard progressive flags `1` (escape-code disambiguation) and `8` (report all keys as CSI-u). Swing preserves press/repeat/release for AWT-visible non-text physical keys without per-event allocation. Rich native layout, IME, alternate-key, and associated-text reporting (`2`, `4`, `16`) is explicitly deferred and never advertised by portable hosts.
