...
- Compatible terminal apps can now read clipboard text, including Unicode and multiple lines, when permitted, with consistent, keyboard-accessible confirmation dialogs to approve once or block requests from that terminal. New settings ask before reading; saved permission choices are preserved.
