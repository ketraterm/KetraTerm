...
/**
 * Window-scoped owner of bounded background directory completion work.
 *
 * Providers created by this service are session-local so request generations
 * and snapshots cannot leak across terminal tabs. Worker capacity and queued
            ...
            executor.execute {
                ...
                synchronized(lock) {
                    ...
                    if (!closed.get()) {
                        ...
                        trimSnapshots()
                        publish = requestedGeneration != null && activeKey == key && activeGeneration == requestedGeneration
                    }
                }
                if (publish) {
                    try {
                        ...
                        // The owning pane may be closing while a ready snapshot is published.
                    }
                }
            }
        } catch (_: RuntimeException) {
...
/** Resolves host paths without filesystem access. */
internal class StandalonePathResolver(
...
internal fun interface StandaloneDirectoryScanner {
    fun scan(
        directory: Path,
        entryNamePrefix: String,
...
/**
 * Best-effort time- and entry-bounded local directory scanner.
 *
 * Individual filesystem calls are not guaranteed interruptible by NIO, so the
 * deadline is enforced between entries. The bounded worker pool still keeps
 * all such I/O off the EDT and limits concurrent blocked providers.
 */
internal class BoundedStandaloneDirectoryScanner(
